...
/* Placeholder artwork data until Sanity is populated */
const artworks = [
  { id:1, title:"Early Morning Breakfast", medium:"Acrylic on Canvas", dims:"24 × 36 in", price:1800, status:"available", category:"Fine Art", year:2024, color:"#8B6F5C" },
  { id:2, title:"Victorian Dawn", medium:"Watercolor on Paper", dims:"18 × 24 in", price:1200, status:"available", category:"Watercolor", year:2023, color:"#7A8E72" },
  { id:3, title:"Old Florida", medium:"Watercolor on Paper", dims:"16 × 20 in", price:950, status:"available", category:"Watercolor", year:2022, color:"#B8956B" },
  { id:4, title:"Emotional Currents", medium:"Mixed Media on Canvas", dims:"24 × 36 in", price:1800, status:"available", category:"Mixed Media", year:2024, color:"#6B7F8E" },
  { id:5, title:"Second Life", medium:"Recycled Mixed Media", dims:"20 × 24 in", price:750, status:"available", category:"Mixed Media", year:2023, color:"#8E7B6B" },
  { id:6, title:"Horizon Line", medium:"Watercolor on Paper", dims:"12 × 36 in", price:1400, status:"available", category:"Watercolor", year:2024, color:"#6B8E7F" },
  { id:7, title:"Storm Hill Memory", medium:"Acrylic on Board", dims:"16 × 20 in", price:900, status:"sold", category:"Fine Art", year:2023, color:"#7B6B8E" },
  { id:8, title:"Harbor House Tribute", medium:"Watercolor on Paper", dims:"18 × 24 in", price:1100, status:"available", category:"Watercolor", year:2022, color:"#8E6B7B" },
  { id:9, title:"Daily Study #47", medium:"Acrylic on Board", dims:"8 × 10 in", price:350, status:"available", category:"Daily Painting", year:2024, color:"#6B8E85" },
  { id:10, title:"Winter Park Porch", medium:"Watercolor on Paper", dims:"14 × 18 in", price:800, status:"available", category:"Watercolor", year:2021, color:"#8E8B6B" },
  { id:11, title:"Deltona Sunrise", medium:"Acrylic on Canvas", dims:"20 × 30 in", price:1500, status:"available", category:"Fine Art", year:2024, color:"#C4956B" },
  { id:12, title:"Daily Study #62", medium:"Acrylic on Board", dims:"8 × 10 in", price:350, status:"available", category:"Daily Painting", year:2025, color:"#6B7A8E" },
];

const categories = ["All", "Fine Art", "Watercolor", "Mixed Media", "Daily Painting"];
...
export default function GalleryPage() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      // Hero text reveal
      gsap.from(".gh-line", {
        y: 80, opacity: 0, stagger: 0.08, duration: 1, ease: "power4.out", delay: 0.15
      });
    ...
    <div ref={mainRef}>
      ...
      <section style={{
        ...
        <div style={{maxWidth:1400,margin:"0 auto",width:"100%",position:"relative",zIndex:1}}>
          <div style={{display:"flex",alignItems:"flex-end",justifyContent:"space-between",flexWrap:"wrap",gap:24}}>
            ...
            <div className="gh-count" style={{
              display:"flex",alignItems:"baseline",gap:8,
              fontFamily:"'DM Serif Display',serif",color:"#F5F0E8"
            }}>
          ...
          {categories.map(cat => (
            <button key={cat} onClick={() => setFilter(cat)}
              style={{
                ...
                padding:"18px 24px",cursor:"pointer",whiteSpace:"nowrap",
      ...
      <section style={{background:"#1E1B17",padding:"clamp(40px,5vw,80px) clamp(24px,5vw,80px)",minHeight:"80vh"}}>
        <div style={{
          ...
          {filtered.map((art, i) => {
            const heights = [340,420,300,380,440,320,400,360,280,420,350,390];
            ...
            return (
              <div key={art.id} className="art-card"
                ...
                style={{
                  ...
                  overflow:"hidden",background:art.color,height:h,
                  transition:"transform 0.5s cubic-bezier(0.25,0.8,0.25,1)",
                  transform: isHovered ? "translateY(-4px)" : "none",
                  boxShadow: isHovered ? "0 20px 50px rgba(0,0,0,0.4)" : "0 4px 20px rgba(0,0,0,0.2)",
                }}
              >
                {/* Color field placeholder — will be replaced with real images */}
                <div style={{
                ...
                <div style={{
                  ...
                  <div style={{fontFamily:"'Outfit',sans-serif",fontSize:"0.65rem",fontWeight:500,letterSpacing:"0.15em",textTransform:"uppercase",color:"#C4A86E",marginBottom:6}}>{art.category}</div>
                  <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:"1.4rem",fontWeight:400,color:"#F5F0E8",marginBottom:4}}>{art.title}</div>
                  <div style={{fontFamily:"'Outfit',sans-serif",fontSize:"0.72rem",color:"#B8AFA3",letterSpacing:"0.05em"}}>{art.medium} · {art.dims}</div>
                  <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginTop:14}}>
                    <span style={{fontFamily:"'Outfit',sans-serif",fontSize:"0.95rem",fontWeight:600,color:"#F5F0E8"}}>
                      {art.status === "sold" ? "Sold" : `$${art.price.toLocaleString()}`}
                    </span>
                    {art.status === "available" && (
                      <span style={{fontFamily:"'Outfit',sans-serif",fontSize:"0.65rem",fontWeight:500,letterSpacing:"0.12em",textTransform:"uppercase",color:"#C47D5A",border:"1px solid #C47D5A",padding:"6px 14px",transition:"all 0.3s"}}>Inquire</span>
                    )}
                    {art.status === "sold" && (
      ...
      {selectedArt && (
        <div onClick={() => setSelectedArt(null)} style={{
          ...
          <div onClick={e => e.stopPropagation()} style={{
            ...
            background:"#2A2520",overflow:"hidden",cursor:"default",
            animation:"slideUp 0.4s ease",
          }}>
            {/* Art preview */}
            <div style={{aspectRatio:"3/4",background:selectedArt.color,display:"flex",alignItems:"center",justifyContent:"center",position:"relative"}}>
              <div style={{background:`linear-gradient(135deg, ${selectedArt.color}, ${selectedArt.color}cc)`,position:"absolute",inset:0}} />
              <span style={{position:"relative",fontFamily:"'Cormorant Garamond',serif",fontStyle:"italic",color:"rgba(255,255,255,0.3)",fontSize:"1.1rem"}}>{selectedArt.medium}</span>
            </div>
            {/* Details panel */}
            <div style={{padding:"48px 40px",display:"flex",flexDirection:"column",justifyContent:"center"}}>
              <div style={{fontFamily:"'Outfit',sans-serif",fontSize:"0.65rem",fontWeight:500,letterSpacing:"0.15em",textTransform:"uppercase",color:"#C4A86E",marginBottom:12}}>{selectedArt.category} · {selectedArt.year}</div>
              <h2 style={{fontFamily:"'DM Serif Display',serif",fontSize:"2rem",fontWeight:400,color:"#F5F0E8",marginBottom:8}}>{selectedArt.title}</h2>
              <p style={{fontFamily:"'Outfit',sans-serif",fontSize:"0.85rem",color:"#B8AFA3",marginBottom:24}}>{selectedArt.medium} · {selectedArt.dims}</p>
              ...
              <p style={{fontFamily:"'Cormorant Garamond',serif",fontSize:"1.05rem",fontWeight:300,fontStyle:"italic",color:"#D4C9B8",lineHeight:1.7,marginBottom:32}}>
                Each piece is an expression of emotion — a method of communication, heart and soul on canvas.
              </p>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:24}}>
                <span style={{fontFamily:"'DM Serif Display',serif",fontSize:"1.8rem",color:"#F5F0E8"}}>
                  {selectedArt.status === "sold" ? "Sold" : `$${selectedArt.price.toLocaleString()}`}
                </span>
                {selectedArt.status === "available" && (
                  <a href="/#contact" style={{
                    ...
                    padding:"14px 28px",textDecoration:"none",transition:"all 0.3s",display:"inline-block",
                  }}>Purchase / Inquire</a>
                )}
              </div>
              <button onClick={() => setSelectedArt(null)} style={{
                ...
                cursor:"pointer",padding:0,marginTop:"auto",transition:"color 0.3s",
              }}
